    ...
    <section id="projects" className="min-h-screen flex items-center justify-center bg-black py-20 scroll-mt-16" data-animation="animate-fade-in">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-10" data-stagger>
          {categories.map((category, index) => (
            <button
              ...
              className={`magical-card group relative p-6 rounded-xl border-2 transition-all duration-300 hover:scale-105 ${
                activeCategory === category.name
                  ? `bg-gradient-to-br from-gray-800 to-gray-900 border-${category.color.split(' ')[0].split('-')[1]}-500 shadow-lg shadow-${category.color.split(' ')[0].split('-')[1]}-500/25 scale-105`
                  : 'bg-gradient-to-br from-gray-900/50 to-gray-800/50 border-gray-700 hover:border-gray-600 opacity-60 hover:opacity-80'
              }`}
            >
              {/* Active state glow effect */}
              {activeCategory === category.name && (
                <div className={`absolute inset-0 bg-gradient-to-br ${category.color} rounded-xl opacity-20 animate-pulse`}></div>
              )}
              
              <div className={`absolute inset-0 bg-gradient-to-br ${category.color} rounded-xl transition-opacity duration-300 ${
                activeCategory === category.name ? 'opacity-15' : 'opacity-0 group-hover:opacity-5'
              }`}></div>

              <div className="relative z-10 flex flex-col items-center text-center">
                <div className={`inline-flex p-3 rounded-lg bg-gradient-to-br ${category.color} mb-3 shadow-lg transition-all duration-300 ${
                  activeCategory === category.name ? 'scale-125 shadow-xl shadow-cyan-500/25' : 'group-hover:scale-110'
                }`}>
                ...
                <h3 className={`text-sm font-bold transition-colors duration-300 ${
                  activeCategory === category.name ? 'text-white drop-shadow-lg' : 'text-gray-400 group-hover:text-gray-200'
                }`}>
                  {category.name}
                </h3>
                ...
                {activeCategory === category.name && (
                  <div className={`absolute -top-1 -right-1 w-3 h-3 bg-gradient-to-br ${category.color} rounded-full animate-pulse shadow-lg`}></div>
                )}
              </div>
            </button>
          ))}
        ...
        <div className="mb-6 flex items-center justify-center">
          <div className="flex items-center gap-3 px-6 py-3 bg-gradient-to-r from-gray-800/50 to-gray-900/50 rounded-full border border-gray-700">
            <div className={`w-3 h-3 rounded-full bg-gradient-to-br ${categories.find(cat => cat.name === activeCategory)?.color} animate-pulse`}></div>
            <span className="text-sm font-medium text-gray-300">
              Showing {filteredProjects.length} project{filteredProjects.length !== 1 ? 's' : ''} from <span className="text-cyan-400 font-bold">{activeCategory}</span>
            </span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {filteredProjects.map((project, index) => {
            ...
            return (
              <div
                key={index}
                className="magical-card group bg-gradient-to-br from-gray-900 to-gray-800 rounded-xl border border-gray-700 hover:border-gray-600 overflow-hidden transition-all duration-300 hover:scale-105 relative"
                data-stagger
              >
                {/* Category indicator ribbon */}
                <div className={`absolute top-4 right-4 z-10 px-3 py-1 bg-gradient-to-r ${categoryInfo?.color} rounded-full shadow-lg`}>
                ...
                <div className="relative overflow-hidden h-40">
                  ...
                  <div className={`absolute inset-0 bg-gradient-to-br ${categoryInfo?.color} opacity-5 group-hover:opacity-10 transition-opacity duration-300`}></div>
                </div>
